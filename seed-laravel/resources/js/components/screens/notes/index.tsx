...
export const Notes = ({ notes, title }: { notes: TNote[]; title: string }) => {
    return (
        <main className="mx-auto flex w-full max-w-7xl flex-1 flex-col space-y-6 overflow-hidden">
            ...
            <div className="flex-1 space-y-6 overflow-y-auto pr-2.5 pb-8 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-border [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-muted">
                {notes.length === 0 ? (
                    <NoNotesMessage />
                ) : (
